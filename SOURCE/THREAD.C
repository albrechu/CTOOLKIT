...
PMUTEX LoadMutex(PALLOCATOR allocator)
{
   ...
   PMUTEX mutex = (PMUTEX)Calloc(allocator, sizeof(MUTEX), alignof(MUTEX));
   if (not mutex) return null;
   mutex->Parent = *allocator;
#if defined(_WIN32)
...
{
   if (not allocator) allocator = DefaultAllocator();
   PCONDITIONVARIABLE cv = (PCONDITIONVARIABLE)Calloc(allocator, sizeof(CONDITIONVARIABLE), alignof(CONDITIONVARIABLE));
...
#endif
   Free(&condition_variable->Parent, condition_variable);
}
...
PCOUNTLATCH LoadCountLatch(PALLOCATOR allocator)
{
   ...
   // Load Latch
   if (not allocator) allocator = DefaultAllocator();
   PCOUNTLATCH latch = (PCOUNTLATCH)Calloc(allocator, sizeof(COUNTLATCH), alignof(COUNTLATCH));
   if (not latch) return null;
   latch->Parent = *allocator;
...
{
   if (not allocator) allocator = DefaultAllocator();
   PTHREAD thread = (PTHREAD)Calloc(allocator, sizeof(THREAD), alignof(THREAD));
   if (not thread) return null;
   thread->Parent   = *allocator;
   ...
   *handle = CreateThread(null, 0, ThreadStartWrapper, thread, 0, null);
   if (*handle == null)
	  return Free(allocator, thread), null;
   return thread;
#elif defined(__unix__) || defined(__APPLE__)
...
PJOBPOOL LoadJobPool(PALLOCATOR allocator, U32 threads_count, U32 job_capacity)
{
   errdfs(6);

   if (not allocator) allocator = DefaultAllocator();
   PJOBPOOL job_pool = (PJOBPOOL)Calloc(allocator, sizeof(JOBPOOL), alignof(JOBPOOL));
   if (not job_pool) return null;
   memzero(job_pool);
...
// JOB GRAPH                           /
// / / / / / / / / / / / / / / / / / / /
PJOBGRAPH LoadJobGraph(PALLOCATOR allocator, PJOBPOOL job_pool)
{
   errdfs(5);

   if (not allocator) allocator = DefaultAllocator();
   PJOBGRAPH graph = (PJOBGRAPH)Calloc(allocator, sizeof(JOBGRAPH), alignof(JOBGRAPH));
   if (not graph) return null;
   errdf(Free, allocator, graph);
   graph->Parent = *allocator;
...
{
   Free(&job_graph->Parent, job_graph->Commands);
   Free(&job_graph->Parent, job_graph->JobStack);
   Free(&job_graph->Parent, job_graph->JobGroups);
   Free(&job_graph->Parent, job_graph->Stack);
   FreeCountLatch(job_graph->WaitLatch);
   Free(&job_graph->Parent, job_graph);
}
...
VOID JobGraphRun(PJOBGRAPH job_graph)
{
   ...
   if (job_graph->JobsStackCapacity < job_graph->CommandsCount)
   {
	  ...
	  job_graph->JobStack = Realloc(&job_graph->Parent, job_graph->JobStack, sizeof(PJOB) * (U32)(job_graph->JobsStackCapacity * 1.5f), alignof(PJOB));
   }
