...
{
   return allocator->Alloc(allocator->UserData, size, alignment);
}
PVOID Calloc(PALLOCATOR allocator, U64 size, U64 alignment)
{
   PVOID data = allocator->Alloc(allocator->UserData, size, alignment);
   return data ? memset(data, 0, size) : null;
}
PVOID Realloc(PALLOCATOR allocator, PVOID ptr, U64 size, U64 alignment)
{
   return allocator->Realloc(allocator->UserData, ptr, size, alignment);
}
...
PALLOCATOR DefaultAllocator(VOID)
{
   static ALLOCATOR allocator = { .UserData = null, .Alloc = DefaultAlloc, .Realloc = DefaultRealloc, .Free = DefaultFree };
   return &allocator;
}
...
PALLOCATOR ArenaAllocator(PARENA arena)
{
   return &arena->Self;
}
...
// ARENA                               /
// / / / / / / / / / / / / / / / / / / /
PARENA LoadArena(PALLOCATOR allocator, U64 block_size)
{
   if (not allocator) allocator = DefaultAllocator();
   PARENA arena = (PARENA)Calloc(allocator, sizeof(ARENA), alignof(ARENA));
   if (not arena) return null;
   arena->Parent = *allocator;
   arena->DefaultBlockSize = block_size ? block_size : (64 * 1024);
   arena->Self = lit(ALLOCATOR, .UserData = arena, .Alloc = (ALLOC)ArenaPut, .Realloc = ArenaRealloc, .Free = ArenaFree);
   return arena;
}
...
static PVOID ArenaAlignedAlloc(PARENA arena, U64 size, U64 align)
{
   ...
   if ((not block) or ((used_aligned + size) > block->Size)) // No block or insufficient size?
   {
	  ...
	  if (not (new_block = (PARENABLOCK)Alloc(&arena->Parent, sizeof(ARENABLOCK) + block_size, alignof(ARENABLOCK))))
		 return null;
...
static PPOOLBLOCK PoolBlockCreate(PPOOL P, U64 ObjSize, U64 BlockCap)
{
   ...
   PPOOLBLOCK B = (PPOOLBLOCK)Alloc(&P->Parent, Total, POOL_ALIGN);
   if (!B) return null;
...
PVOID PoolAlloc(PVOID user_data, U64 size, U64 alignment)
{
   ...
   CAssert(PooSameAlignedObjSize(pool, size) and "Cannot allocate an object that differs in size from the pools object size.");
   return PoolPut(pool);
}
PVOID PoolRealloc(PVOID user_data, PVOID ptr, U64 size, U64 alignment)
{
   _ = user_data; _ = ptr; _ = size; _ = alignment;
   return null;
}
...
PALLOCATOR PoolAllocator(PPOOL pool)
{
   return &pool->Self;
}
PPOOL LoadPool(PALLOCATOR allocator, U64 ObjSize, U64 BlockCap)
{
...
#endif
   P->Self = lit(ALLOCATOR, .UserData = P, .Alloc = PoolAlloc, .Realloc = PoolRealloc, .Free = PoolFree);
   return P;
}
...
PVOID PoolNextRev(PPOOLITER It)
{
   ...
   while (It->Block)
   {
	  ...
	  else if (It->List == 0)
	  {
		 It->List = 1;
		 PPOOLBLOCK Tail = null;
		 for (PPOOLBLOCK C = pool->Partial; C; C = C->Next)
			Tail = C;
		 It->Block = Tail;
	  }
	  else
