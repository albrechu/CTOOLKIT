...
static BOOL ParseValue(ARGTYPE type, PCHAR string, PVOID value)
{
   ...
   case ARGTYPE_PATH:
   {
	  SV path = svcstr(string);
	  path = PathAbs(path, value);
	  return svvalid(path);
   }
   case ARGTYPE_CSTR:
   {
...
// COMMANDS                            /
// / / / / / / / / / / / / / / / / / / /
RESULT Run(PALLOCATOR allocator, U32 args_count, PSV args)
{
   SV cmd = SVJoinList(allocator, ' ', args_count, args);
   int result = system(cmd.String);
   Free(allocator, (PVOID)cmd.String);
   switch (result)
   {
   case 0:
	  return OK;
...
// PROCESS                             /
// / / / / / / / / / / / / / / / / / / /
PPROCESS LoadProcess(PALLOCATOR allocator, PROCESSMODE mode, U32 args_count, PSV args)
{
   errdfs(16);
   // Join Arguments
   SV cmd = SVJoinList(allocator, ' ', args_count, args);
   if (svinvalid(cmd)) return null;
   errdf(Free, allocator, (PVOID)cmd.String);
   // Allocate process
   ...
   if (not CreateProcessA(null, cmd.String, null, null, true, 0, null, null, &si, &pi))
   {
