...
RESULT LoadFile(PALLOCATOR allocator, SV path, FILEMODE mode, U64 initial_capacity, PPFILEDATA file)
{
   errdfs(2);
   if (not allocator) allocator = DefaultAllocator();
   PFILEDATA f = (PFILEDATA)Calloc(allocator, sizeof(FILEDATA), alignof(FILEDATA));
   if (not f) return OUT_OF_HOST_MEMORY;
   errdf(Free, allocator, f);
   f->Allocator = *allocator;
   if (mode != FILEMODE_NONE)
   {
	  if (svinvalid(path))
		 return errdfflush(), UNDEFINED_ERROR;

	  RESULT result = FileOpen(f, path, mode);
	  if (result != OK)
		 return errdfflush(), result;
   }

   if (initial_capacity)
   {
	  if (not (f->Buffer.String = Alloc(allocator, initial_capacity, sizeof(U8))))
		 return errdfflush(), OUT_OF_HOST_MEMORY;
	  f->Buffer.Size    = 0;
	  f->BufferCapacity = initial_capacity;
   }
   *file = f;
   return OK;
}
VOID FreeFile(PFILEDATA file)
{
   if (file->File)
	  FileClose(file);
...
RESULT FileOpen(PFILEDATA file, SV path, FILEMODE mode)
{
