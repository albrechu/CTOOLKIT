...
static inline U64 XXHRotl64(U64 x, int r)
{
    return (x << r) | (x >> (64ULL - r));
}
static inline U64 XXH3Avalanche(U64 h)
{
    h ^= h >> 37ull;
    h *= XXH_PRIME64_3;
    h ^= h >> 32ull;
...
static inline U64 XXH3Mix(U64 v1, U64 v2)
{
    U64 h = v1 ^ XXHRotl64(v2 + v1, 31);
    return XXH3Avalanche(h);
}
// XX3 hash
U64 Hash64(FDATA data, U64 seed)
{
    const U8 *p = (const U8 *)data.data;
    U64 h = seed + XXH_PRIME64_5 + data.size;
    if (data.size <= 16) // handle short inputs directly
    {
        if (data.size >= 8)
        {
            U64 v1 = *(PU64)p;
            U64 v2 = *(PU64)(p + data.size - 8);
            h ^= XXH3Mix(v1 + seed, v2 ^ seed);
            return XXH3Avalanche(h);
        }
        if (data.size >= 4)
        {
            U32 v1 = *(PU32)p;
            U32 v2 = *(PU32)(p + data.size - 4);
            h ^= (U64)v1 + ((U64)v2 << 32);
            return XXH3Avalanche(h);
        }
        if (data.size > 0)
    ...
    // long inputs
    const U64 *pl = (const U64 *)p;
    const U64 *end = (const U64 *)(p + (data.size & ~15ULL));
    while (pl < end)
    {
        U64 v1 = *pl++;
        U64 v2 = *pl++;
        h ^= XXH3Mix(v1, v2);
        h = XXHRotl64(h, 27) * XXH_PRIME64_1 + XXH_PRIME64_4;
    }
    // last 0..15 bytes
    p = (const U8 *)pl;
    U64 remaining = data.size & 15;
...
// Hashing
static U32 const crc32c_table[][256] =
{
...
static U32 HashSWCrc32c(U32 crc, const PVOID buf, U64 len)
{
    const U8 *data = (const U8 *)buf;
    while (len && ((U64)data & 7) != 0)
    {
    ...
    for (U64 i = 0; i < n; i++)
    {
        U64 word = crc ^ ((const PU64)data)[i];
        crc = crc32c_table[7][word & 0xffull] ^
            crc32c_table[6][(word >> 8ull) & 0xffull] ^
            crc32c_table[5][(word >> 16ull) & 0xffull] ^
            crc32c_table[4][(word >> 24ull) & 0xffull] ^
            crc32c_table[3][(word >> 32ull) & 0xffull] ^
            crc32c_table[2][(word >> 40ull) & 0xffull] ^
            crc32c_table[1][(word >> 48ull) & 0xffull] ^
            crc32c_table[0][word >> 56ull];
    }
    data += n << 3;
    len &= 7;
    while (len)
...
#ifdef WAS_COMPILED_WITH_SSE42
static U32 HashHWCrc32c(U32 crc, const void *buf, U64 len)
{
    const unsigned char *next = (const unsigned char *)buf;
    // Pre-process
    U64 crc0 = crc ^ 0xFFFFFFFFu;
#if defined(__x86_64__) || defined(_M_X64)
    while (len && ((uptr)next & 7)) // Align to 8 bytes
    {
        crc0 = _mm_crc32_U8((U32)crc0, *next);
        next++;
        len--;
    }
    while (len >= 8) // Process 8 bytes at a time
    {
        U64 val;
        memcpy(&val, next, sizeof(U64)); // safe unaligned load
        crc0 = _mm_crc32_U64(crc0, val);
        next += 8;
        len -= 8;
    }
#else
    while (len >= 4) // 32-bit fallback
    {
        U32 val;
        __builtin_memcpy(&val, next, sizeof(U32));
        crc0 = _mm_crc32_U32((U32)crc0, val);
        next += sizeof(U32);
        len -= sizeof(U32);
    }
#endif
    while (len) // Remaining bytes
    {
        crc0 = _mm_crc32_U8((U32)crc0, *next);
        next++;
        len--;
...
U32 Crc32(U32 crc, const PVOID buf, U64 len)
{
    ...
    BOOL has_sse42;
#if defined(__x86_64__) || defined(_M_X64)
    has_sse42 = true; // guaranteed on x64
    ...
    #endif
    has_sse42 = (info[2] & (1 << 20)) != 0; // SSE4.2 bit
#else
    has_sse42 = false;
#endif
    return has_sse42 ? HashHWCrc32c(crc, buf, len) : HashSWCrc32c(crc, buf, len);
#else
    return HashSWCrc32c(crc, buf, len);
#endif
}

typedef struct RANS64ENCODERSYMBOL
{
    U64 rcp_freq;  // Fixed-point reciprocal frequency
    U32 freq;      // Symbol frequency
    U32 bias;      // Bias
    U32 cmpl_freq; // Complement of frequency: (1 << scale_bits) - freq
    U32 rcp_shift; // Reciprocal shift
} RANS64ENCODERSYMBOL, *PRANS64ENCODERSYMBOL;

typedef struct RANS64DECODERSYMBOL
{
    U32 start; // Start of range.
    U32 freq;  // Symbol frequency.
} RANS64DECODERSYMBOL;

typedef U64 RANS64STATE, *PRANS64STATE;

#if defined(_MSC_VER)
static inline U64 Rans64MulHi(U64 a, U64 b) { return __umulh(a, b); }
#elif defined(__GNUC__)
static inline U64 Rans64MulHi(U64 a, U64 b) { return (U64)(((unsigned __int128)a * b) >> 64); }
#else
#error Unknown/unsupported compiler!
#endif
static inline void Rans64EncPutSymbol(PRANS64STATE r, PPU32 pptr, const PRANS64ENCODERSYMBOL sym, U32 scale_bits)
{
#define RANS64_L (1ull << 31)
    // Renormalize
    U64 x = *r;
    U64 x_max = ((RANS64_L >> scale_bits) << 32) * sym->freq; // turns into a shift
    if (x >= x_max)
    {
        *pptr -= 1;
        **pptr = (U32)x;
    ...
    // x = C(s,x)
    U64 q = Rans64MulHi(x, sym->rcp_freq) >> sym->rcp_shift;
    *r = x + sym->bias + q * sym->cmpl_freq;
}

static inline void Rans64EncFlush(RANS64STATE *r, PPU32 pptr)
{
    U64 x = *r;
    *pptr -= 2;
    (*pptr)[0] = (U32)(x >> 0ull);
...
static inline void Rans64EncodersSymbolInit(RANS64ENCODERSYMBOL *s, U32 start, U32 freq, U32 scale_bits)
{
    s->freq = freq;
...
static inline void RAns64DecoderSymbolInit(RANS64DECODERSYMBOL *s, U32 start, U32 freq)
{
    s->start = start;
    s->freq = freq;
}
static inline U32 Rans64DecGet(RANS64STATE *r, U32 scale_bits)
{
    return *r & ((1u << scale_bits) - 1);
}

typedef enum DECODESTRATEGY_
{
    DECODESTRATEGY_LINEAR,
    DECODESTRATEGY_BINARY,
    DECODESTRATEGY_TABLE,

    DECODESTRATEGY_COUNT,
} DECODESTRATEGY;

typedef struct COMPRESSEDRANS
{
    U64 uncompressed_size;
    U64 rans_final_state;
    U8  decoding_strategy;
    U8  scale_bits;
    U8  reserved0;
    U8  reserved1;
    U32 frequency_table[256];
    U8  data[];
} COMPRESSEDRANS, *PCOMPRESSEDRANS;

static inline I32 Log2_32(U32 n)
{
#if defined(_MSC_VER)
    unsigned long index;
...
FDATA RAnsEncode(ALLOCATOR allocator, FDATA uncompressed, U8 scaling_bits)
{
    ...
    U8 *bytes = (U8 *)uncompressed.data;
    for (U64 i = 0; i < uncompressed.size; i++)
    {
        frequencies[bytes[i]]++;
    }
    // Calculate cumulative frequencies and symbol count
    U32 cumulative_frequencies[257]; // Last one is total
    cumulative_frequencies[0] = 0;
    U32 alphabet_cardinatlity = 0;
    for (U32 i = 0; i < 256; i++)
    {
        alphabet_cardinatlity += frequencies[i] > 0;
        cumulative_frequencies[i + 1] = cumulative_frequencies[i] + frequencies[i];
    }
    // Normalize
    U32 scaling = 1 << scale_bits;
    U32 total = cumulative_frequencies[256];
    for (U32 i = 1; i <= 256; i++)
        cumulative_frequencies[i] = ((U64)scaling * cumulative_frequencies[i]) / total;

    for (U32 i = 0; i < 256; i++)
    {
        if (frequencies[i] and cumulative_frequencies[i + 1] == cumulative_frequencies[i])
        {
            U32 best_freq = ~0u;
            I64 best_steal = -1;
            for (I64 j = 0; j < 256; j++)
            {
                U32 freq = cumulative_frequencies[j + 1] - cumulative_frequencies[j];
            ...
            if (best_steal < i)
            {
                for (I64 j = best_steal + 1; j <= i; j++)
                    cumulative_frequencies[j]--;
            }
            else
            {
                for (I64 j = i + 1; j <= best_steal; j++)
                    cumulative_frequencies[j]++;
            }
        }
    }
    ...
    for (U32 i = 0; i < 256; i++)
    {
        frequencies[i] = cumulative_frequencies[i + 1] - cumulative_frequencies[i];
    }

    RANS64ENCODERSYMBOL encoding_symbols[256];
    for (U32 i = 0; i < 256; i++)
    {
        Rans64EncodersSymbolInit(&encoding_symbols[i], cumulative_frequencies[i], frequencies[i], scale_bits);
    }

    // Choose decoding strategy 
    U32 strategy;
    U64 bitstream_max = uncompressed.size * ((scale_bits / 8) + 1) + 64;
    U64 decoding_size = sizeof(COMPRESSEDRANS) + bitstream_max;
    if (alphabet_cardinatlity <= 8)
    {
        strategy = DECODESTRATEGY_LINEAR;
    }
    else
    {
        U32 search_cost = Log2_32(alphabet_cardinatlity);
        if (scaling > (uncompressed.size * search_cost))
        {
            strategy = DECODESTRATEGY_BINARY;
        }
        else
        {
            strategy = DECODESTRATEGY_TABLE;
            decoding_size += scaling;
        }
    }

    PCOMPRESSEDRANS compressed    = (PCOMPRESSEDRANS)Alloc(&allocator, decoding_size, alignof(COMPRESSEDRANS));
    compressed->uncompressed_size = uncompressed.size;
    compressed->decoding_strategy = strategy;
    compressed->scale_bits        = scaling_bits;
    memcpy(compressed->frequency_table, frequencies, sizeof(compressed->frequency_table));

    U8 *compressed_start;
    if (strategy == DECODESTRATEGY_TABLE)
    {
        U8 *cum2sym = (U8 *)compressed->data;
        compressed_start = (U8 *)compressed->data + scaling;
        for (U32 s = 0; s < 256; s++)
            for (U32 i = cumulative_frequencies[s]; i < cumulative_frequencies[s + 1]; i++)
                cum2sym[i] = s;
    }
    else
    {
        compressed_start = (U8 *)compressed->data;
    }
    PU32 end = (PU32)((U8 *)compressed + decoding_size);
    PU32 ptr = end;

    U64 state = RANS64_L;
    for (U64 i = uncompressed.size; i > 0; i--)
    {
        Rans64EncPutSymbol(&state, &ptr, &encoding_symbols[bytes[i - 1]], scaling_bits);
    }
    Rans64EncFlush(&state, &ptr);
    compressed->rans_final_state = state;
    // Calculate compressed and move to start.
    U64 written = (U8 *)end - (U8 *)ptr;
    memmove(compressed_start, ptr, written);
    // Reduce size
    U64 reduced_size = (compressed_start - (U8 *)compressed) + written;
    compressed = (COMPRESSEDRANS *)Realloc(&allocator, compressed, reduced_size, alignof(PCOMPRESSEDRANS));

    FDATA compressed_data = fdata(compressed, reduced_size);
    return compressed_data;
}

static inline void Rans64DecInit(PRANS64STATE r, PPU32 pptr)
{
    U64 x = (U64)((*pptr)[0]) << 0;
    x |= (U64)((*pptr)[1]) << 32;
    *pptr += 2;
...
static inline void Rans64DecAdvance(PRANS64STATE r, PPU32 pptr, U32 start, U32 freq, U32 scale_bits)
{
    U64 mask = (1ull << scale_bits) - 1;
    U64 x = freq * ((*r) >> scale_bits) + ((*r) & mask) - start;
    if (x < RANS64_L) // renormalize
    {
        x = (x << 32) | **pptr;
        *pptr += 1;
    }
...
static inline void Rans64DecAdvanceSymbol(PRANS64STATE r, PPU32 pptr, RANS64DECODERSYMBOL const *sym, U32 scale_bits)
{
...
FDATA RAnsDecode(ALLOCATOR allocator, FDATA compressed)
{
    ...
    PU32 ptr = (PU32)&compressed_rans->data[strategy == DECODESTRATEGY_TABLE ? scaling : 0];

    U32 cumulative_frequencies[257]; // Last one is total
    cumulative_frequencies[0] = 0;
    for (U32 i = 0; i < 256; i++)
    {
        cumulative_frequencies[i + 1] = cumulative_frequencies[i] + compressed_rans->frequency_table[i];
    }

    RANS64DECODERSYMBOL dsyms[256];
    for (U32 i = 0; i < 256; i++)
    {
        RAns64DecoderSymbolInit(&dsyms[i], cumulative_frequencies[i], compressed_rans->frequency_table[i]);
    }

    U64 state = compressed_rans->rans_final_state;
    Rans64DecInit(&state, &ptr);
    U8 *decoded = (U8 *)Alloc(&allocator, compressed_rans->uncompressed_size, alignof(U8));
    switch (compressed_rans->decoding_strategy)
    {
    case DECODESTRATEGY_LINEAR:
    {
        for (U64 i = 0; i < compressed_rans->uncompressed_size; i++)
        {
            U32 cum = Rans64DecGet(&state, scale_bits);
            U32 symbol = 0;
            while (cumulative_frequencies[symbol + 1] <= cum)
            {
                symbol++;
            }
            decoded[i] = symbol;
            Rans64DecAdvanceSymbol(&state, &ptr, &dsyms[symbol], scale_bits);
        }
        break;
    }
    case DECODESTRATEGY_BINARY:
    {
        for (U64 i = 0; i < compressed_rans->uncompressed_size; i++)
        {
            U32 cum = Rans64DecGet(&state, scale_bits);
            ...
            while (low < high)
            {
                ...
                if (cumulative_frequencies[mid] <= cum)
                    low = mid;
                else
                    high = mid - 1;
            }
            decoded[i] = low;
            Rans64DecAdvanceSymbol(&state, &ptr, &dsyms[low], scale_bits);
        }
        break;
    }
    case DECODESTRATEGY_TABLE:
    {
        U8 *cum2sym = (U8 *)compressed_rans->data;
        for (U64 i = 0; i < compressed_rans->uncompressed_size; i++)
        {
            U8 symbol = cum2sym[Rans64DecGet(&state, scale_bits)];
            decoded[i] = symbol;
            Rans64DecAdvanceSymbol(&state, &ptr, &dsyms[symbol], scale_bits);
        }
        break;
    ...
    return fdata(decoded, compressed_rans->uncompressed_size);
}
void FreeRAns(ALLOCATOR allocator, FDATA data)
{
    Free(&allocator, data.data);
}
