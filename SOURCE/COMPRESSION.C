...
DV RAnsEncode(PALLOCATOR allocator, DV uncompressed, U8 scaling_bits)
{
   CAssert(not dvinvalid(uncompressed) and "Uncompressed data needs to be valid.");

   const U32 scale_bits = scaling_bits;
   ...
   const U32 total = cumulative_frequencies[256];
   for (U32 i = 1; i <= 256; i++)
   ...
   for (U32 i = 0; i < 256; i++)
   {
	  if (frequencies[i] and cumulative_frequencies[i + 1] == cumulative_frequencies[i])
	  {
		 ...
		 for (I64 j = 0; j < 256; j++)
		 {
			const U32 freq = cumulative_frequencies[j + 1] - cumulative_frequencies[j];
   ...
   DV compressed_data = dv(compressed, reduced_size);
...
DV RAnsDecode(PALLOCATOR allocator, DV compressed)
{
   ...
   return dv(decoded, compressed_rans->uncompressed_size);
}
void FreeRAns(PALLOCATOR allocator, DV data)
{
   if (not allocator) allocator = DefaultAllocator();
